...
export const generateNoteFromImages = async (images: UploadedImage[], instruction: string): Promise<string> => {
  ...
  const prompt = `
    ...
    [입력 데이터]
    - 이미지 파일 총 ${images.length}장이 순서대로 제공되었습니다. (1번 ~ ${images.length}번)
    - 사용자 요청사항: "${instruction}"

    [작성 가이드]
    1. 사용자가 특정 이미지(예: "마지막 사진", "6번 사진" 등)를 '스타일'이나 '참고용'으로 지시했다면, 해당 이미지의 문체와 형식을 철저히 따르세요.
    2. 나머지 이미지(활동 사진, 엑셀 기록 등)에서 식별 가능한 활동 날짜, 구체적인 활동 내용, 학생의 역할을 팩트로 추출하여 내용을 구성하세요.
