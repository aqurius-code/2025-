...
export const generateNoteFromImages = async (images: UploadedImage[], instruction: string): Promise<string> => {
  ...
  const prompt = `
    ...
    - 파일 순서대로 "1번 이미지", "2번 이미지", ..., "${images.length}번 이미지"입니다.
    - 사용자 요청사항: "${instruction}"

    [작성 가이드]
    1. **스타일 참고**: 사용자가 특정 번호(예: "6번", "마지막")를 스타일/형식 참고용으로 지정했다면, 해당 이미지의 문체(어미, 문장 길이, 표현 방식)를 철저히 모방하되 내용은 가져오지 마세요.
    2. **내용 추출**: 사용자가 내용 근거로 지정한 이미지 범위(예: "1~5번", "나머지")에서 활동 날짜, 활동명, 학생의 역할, 구체적 행동 등 팩트(Fact)를 추출하세요.
    3. 별도의 스타일 지시가 없다면, 학생의 성장이 드러나는 교육적이고 긍정적인 '생기부 우수 사례' 스타일로 작성하세요.
